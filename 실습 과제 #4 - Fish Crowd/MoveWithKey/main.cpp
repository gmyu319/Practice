...
class MainListener : public FrameListener {
    ...
    Root* mRoot;
    Camera* mCamera;
    SceneNode *mProfessorNode, *mFishNode[50];
    float fishVelocity[50];
...
public:
    MainListener(Root* root, OIS::Keyboard *keyboard) : mKeyboard(keyboard), mRoot(root)
    {
        ...
        for (int i = 0; i < 50; ++i)
        {
            mFishNode[i] = mRoot->getSceneManager("main")->getSceneNode(strFish + to_string(i));
            mFishNode[i]->yaw(Degree(180.0f));
            fishVelocity[i] = rand() % 100 + 50.0f;
        }
    }
    ...
    bool frameStarted(const FrameEvent &evt)
    {
        for (int i = 0; i < 50; ++i)
        {
            if (mFishNode[i]->getPosition().x < -400.f || mFishNode[i]->getPosition().x > 400.0f)
            {
                fishVelocity[i] *= -1;
                mFishNode[i]->yaw(Degree(180.0f));
            }
            mFishNode[i]->translate(fishVelocity[i] * evt.timeSinceLastFrame, 0, 0);
        }
...
public:
    ...
    void go(void)
    {
        ...
        {
            strFish = "Fish";
            entity2[i] = mSceneMgr->createEntity(strFish + to_string(i), "fish.mesh");
            node2[i] = mSceneMgr->getRootSceneNode()->createChildSceneNode(strFish + to_string(i), Vector3(200.0f, 4.0f * i, -200.0f));
            node2[i]->setScale(Vector3(5.0f, 5.0f, 5.0f));
            node2[i]->attachObject(entity2[i]);
        }
